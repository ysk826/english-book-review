        ...
        <div className="min-h-screen flex flex-col">
            ...
            <header className="absolute top-0 left-0 right-0 z-10 flex items-center justify-between px-8 py-5">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Link
                        href="/register"
                        className="px-4 py-2 text-sm font-semibold text-white bg-teal-600 rounded-lg hover:bg-teal-500 transition-colors"
                    >
                        新規登録
                    </Link>
                </div>
            ...
            {/* ヒーローセクション */}
            <section className="bg-slate-900 flex-1 flex items-center min-h-[88vh]">
                <div className="max-w-5xl mx-auto px-8 py-24 w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    ...
                    <div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                            <Link
                                href="/register"
                                className="px-8 py-3.5 text-base font-semibold text-white bg-teal-600 rounded-xl hover:bg-teal-500 transition-colors text-center"
                            >
                                無料ではじめる
                            </Link>
                            <Link
                                href="/login"
                                className="px-8 py-3.5 text-base font-semibold text-slate-300 border border-slate-600 rounded-xl hover:border-slate-400 hover:text-white transition-colors text-center"
                            >
                                ログイン
                            </Link>
                        </div>
                    ...
                    <div className="hidden lg:flex items-center justify-center">
                        <div className="relative w-64 h-72">
                            {/* 一番後ろ */}
                            <div className="absolute inset-0 bg-slate-700 rounded-2xl rotate-6 translate-x-4 translate-y-2 opacity-60" />
                            {/* 中間 */}
                            <div className="absolute inset-0 bg-teal-800 rounded-2xl rotate-3 translate-x-2" />
                            {/* 前面カード */}
                            <div className="absolute inset-0 bg-slate-800 rounded-2xl border border-slate-700 shadow-2xl flex flex-col justify-between p-6">
                                <div>
                                    <div className="w-12 h-1.5 bg-teal-500 rounded mb-3" />
                                    <div className="w-3/4 h-1.5 bg-slate-600 rounded mb-2" />
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center gap-2">
                                        <div className="flex gap-0.5">
                                            {[...Array(5)].map((_, i) => (
                                                <svg key={i} xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill={i < 4 ? '#14b8a6' : '#334155'} viewBox="0 0 256 256">
                                    ...
                                    <div className="w-4/6 h-1.5 bg-slate-700 rounded" />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {/* CTA バナー */}
            <section className="bg-slate-900 py-20 px-8 text-center">
                <h2 className="text-3xl font-bold text-white mb-4">さっそく始めよう</h2>
                <p className="text-slate-400 mb-8">無料で、今すぐ使えます。</p>
                <Link
                    href="/register"
                    className="inline-block px-10 py-4 text-base font-semibold text-white bg-teal-600 rounded-xl hover:bg-teal-500 transition-colors"
                >
                    アカウントを作成する
                </Link>
            </section>

            <footer className="bg-slate-950 text-center py-6 text-xs text-slate-600">
                © 2025 Stride Log
            </footer>
        </div>
    );
